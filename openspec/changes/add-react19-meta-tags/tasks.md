...
- [ ] Document meta tag patterns in project documentation
...
- [ ] Add `<title>` to `/_layout.courses/route.tsx` (课程列表 - Python教学平台)
- [ ] Add `<title>` to `/_layout.courses_.$courseId_.chapters/route.tsx` ([Course Title] - 章节列表)
- [ ] Add `<title>` to `/_layout.courses_.$courseId_.chapters_.$chapterId/route.tsx` ([Chapter Title] - [Course Title])
- [ ] Add `<title>` to `/_layout.courses_.$courseId_.exams/route.tsx` ([Course Title] - 测验列表)
- [ ] Add `<title>` to `/_layout.courses_.$courseId_.exams_.$examId/route.tsx` ([Exam Title] - 测验)
- [ ] Add `<title>` to `/_layout.courses_.$courseId_.exams_.$examId.results/route.tsx` ([Exam Title] - 测验结果)

#### Problem Routes (High Priority for Title + Additional Meta in Phase 2)
- [x] Add `<title>` to `/_layout.problems/route.tsx` (题库 - Python教学平台)
- [ ] Add `<title>` to `/problems.$problemId/route.tsx` ([Problem Title] - 题目详情)
...
- [ ] Add `<title>` to `/jupyter.tsx` (Jupyter Notebook - Python教学平台)
- [ ] Add `<title>` to `/threads.tsx` (讨论区 - Python教学平台)
- [ ] Add `<title>` to `/orders.create.tsx` (创建订单 - Python教学平台)
- [ ] Add `<title>` to `/orders.$order_number.tsx` (订单详情 - Python教学平台)
- [x] Add `<title>` to `/payment.pay.tsx` (支付页面 - Python教学平台)
- [ ] Add `<title>` to `/submission.tsx` (提交详情 - Python教学平台)

### 3.3 Low Priority - Title Only (Phase 1)

#### Problem Sub-routes (13 files)
- [ ] Add `<title>` to `/problems.$problemId/check.tsx` (题目检查)
- [ ] Add `<title>` to `/problems.$problemId/description.tsx` (题目描述)
- [ ] Add `<title>` to `/problems.$problemId/latest_draft.tsx` (最新草稿)
- [ ] Add `<title>` to `/problems.$problemId/mark_as_solved.tsx` (标记已解决)
- [ ] Add `<title>` to `/problems.$problemId/save_draft.tsx` (保存草稿)
- [ ] Add `<title>` to `/problems.$problemId/AlgorithmProblemPage.tsx` (算法题目)
- [ ] Add `<title>` to `/problems.$problemId/ChoiceProblemPage.tsx` (选择题页面)
- [ ] Add `<title>` to `/problems.$problemId/FillBlankProblemPage.tsx` (填空题页面)

#### Utility Routes (3 files)
- [ ] Add `<title>` to `/refresh.tsx` (刷新令牌)
- [ ] Add `<title>` to `/upload.$type.tsx` (文件上传)
- [ ] Add `<title>` to `/upload.$type.$.tsx` (文件上传详情)

### 3.3 Main Navigation Routes (9 files)

- [ ] Add meta tags to `/_layout.performance.tsx` (学习数据分析 - Python教学平台)
- [ ] Add meta tags to `/_layout.playground.tsx` (代码练习场 - Python教学平台)
- [ ] Add meta tags to `/_layout.problems.tsx` (题库 - Python教学平台)
- [ ] Add meta tags to `/_layout.threads_.$threadId.tsx` ([Thread Title] - 讨论区 - Python教学平台)
- [ ] Add meta tags to `/jupyter.tsx` (Jupyter Notebook - Python教学平台)
- [ ] Add meta tags to `/orders.$order_number.tsx` (订单详情 - Python教学平台)
- [ ] Add meta tags to `/orders.create.tsx` (创建订单 - Python教学平台)
- [ ] Add meta tags to `/payment.pay.tsx` (支付页面 - Python教学平台)
- [ ] Add meta tags to `/threads.tsx` (讨论区 - Python教学平台)

### 3.4 Course and Problem Routes (13 files)

- [ ] Add meta tags to `/_layout.courses/route.tsx` (课程列表 - Python教学平台)
- [ ] Add meta tags to `/problems.$problemId/check.tsx` (题目检查 - Python教学平台)
- [ ] Add meta tags to `/problems.$problemId/description.tsx` (题目描述 - Python教学平台)
- [ ] Add meta tags to `/problems.$problemId/latest_draft.tsx` (最新草稿 - Python教学平台)
- [ ] Add meta tags to `/problems.$problemId/mark_as_solved.tsx` (标记已解决 - Python教学平台)
- [ ] Add meta tags to `/problems.$problemId/save_draft.tsx` (保存草稿 - Python教学平台)
- [ ] Add meta tags to `/problems.$problemId/submissions.tsx` (提交记录 - Python教学平台)
- [ ] Add meta tags to `/problems.$problemId/AlgorithmProblemPage.tsx` (算法题目 - Python教学平台)
- [ ] Add meta tags to `/problems.$problemId/ChoiceProblemPage.tsx` (选择题页面 - Python教学平台)
- [ ] Add meta tags to `/problems.$problemId/FillBlankProblemPage.tsx` (填空题页面 - Python教学平台)

### 3.5 Upload and Utility Routes (3 files)

- [ ] Add meta tags to `/refresh.tsx` (刷新令牌 - Python教学平台)
- [ ] Add meta tags to `/submission.tsx` (提交详情 - Python教学平台)
- [ ] Add meta tags to `/upload.$type.tsx` (文件上传 - Python教学平台)
- [ ] Add meta tags to `/upload.$type.$.tsx` (文件上传详情 - Python教学平台)
...
### Phase 1 Testing
- [ ] Verify all 41 routes have correct `<title>` tags
- [ ] Check browser tab titles display correctly
- [ ] Verify SSR renders titles correctly (view-source)
- [ ] No console warnings

### Phase 2 Testing
- [ ] Test social media sharing for high priority routes
- [ ] Verify descriptions are 120-160 characters
- [ ] Verify titles are under 60 characters
- [ ] Test with dynamic data and fallbacks

## 5. Testing and Validation

- [ ] Verify meta tags render correctly during SSR (check view-source)
- [ ] Test social media sharing with Facebook Sharing Debugger
- [ ] Test Open Graph tags with LinkedIn Post Inspector
- [ ] Verify no console warnings related to meta tags
- [ ] Check all routes have appropriate titles (browser tab title)
- [ ] Validate title lengths are under 60 characters
- [ ] Validate description lengths are under 160 characters

## 6. Documentation

- [ ] Update React Router section of project.md with meta tag patterns
- [ ] Add examples of meta tag usage to frontend conventions
- [ ] Document dynamic meta tag patterns for future routes
