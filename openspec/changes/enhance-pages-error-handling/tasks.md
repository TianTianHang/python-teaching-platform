...
- [ ] 7.1 手动测试题目列表页的各种错误场景
- [ ] 7.2 手动测试个人资料页的各种错误场景
- [ ] 7.3 手动测试会员页的各种错误场景
- [ ] 7.4 手动测试课程详情页的各种错误场景（特别是部分失败的场景）
- [ ] 7.5 验证所有页面的错误消息都是友好的中文
- [ ] 7.6 验证 401 错误仍然正确重定向到登录页
- [ ] 7.7 验证所有页面的"重新加载"按钮在可重试错误时显示
- [ ] 7.8 验证所有页面的"返回首页"按钮存在
- [ ] 7.9 测试移动端响应式布局（错误 UI 在小屏幕上的显示）
- [ ] 7.10 如果发现问题，修复并重新测试

## 8. 打包和提交

- [ ] 8.1 运行 `git status` 查看所有修改的文件
- [ ] 8.2 运行 `git diff` 确认更改符合预期
- [ ] 8.3 提交更改：`git add` 和 `git commit`
- [ ] 8.4 使用清晰的 commit message 描述改进内容
- [ ] 8.5 运行 `/opsx:archive` 归档此变更
