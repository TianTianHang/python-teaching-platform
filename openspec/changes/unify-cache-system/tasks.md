...
- [ ] 1.5 为 StandardCacheListMixin 添加单元测试
- [ ] 1.6 为 StandardCacheRetrieveMixin 添加单元测试
- [ ] 1.7 创建 `mock_standard_cache_service()` 测试辅助函数

## 2. 迁移courses模块ViewSet

- [x] 2.1 迁移 CourseViewSet 继承到 StandardCacheMixin
- [ ] 2.2 运行 CourseViewSet 单元测试验证迁移
- [x] 2.3 迁移 ExamViewSet 继承到 StandardCacheMixin
- [ ] 2.4 更新 signals.py:137 的 ExamViewSet 缓存失效逻辑使用 CacheInvalidator
- [ ] 2.5 运行 ExamViewSet 单元测试验证迁移
- [x] 2.6 迁移 EnrollmentViewSet 继承到 StandardCacheMixin
- [x] 2.7 验证 EnrollmentViewSet 的 user_id 自动注入逻辑
- [ ] 2.8 更新 signals.py:167 的 EnrollmentViewSet 缓存失效逻辑使用 CacheInvalidator
- [ ] 2.9 运行 EnrollmentViewSet 单元测试验证迁移
- [x] 2.10 迁移 ChapterProgressViewSet 继承到 StandardCacheMixin
- [ ] 2.11 运行 ChapterProgressViewSet 单元测试验证迁移
- [x] 2.12 迁移 ProblemProgressViewSet 继承到 StandardCacheMixin
- [ ] 2.13 运行 ProblemProgressViewSet 单元测试验证迁移
- [x] 2.14 迁移 ExamSubmissionViewSet 继承到 StandardCacheMixin
- [ ] 2.15 运行 ExamSubmissionViewSet 单元测试验证迁移

## 3. 迁移file_management模块ViewSet

- [x] 3.1 迁移 FolderViewSet 继承到 StandardCacheMixin
- [ ] 3.2 运行 FolderViewSet 单元测试验证迁移
- [x] 3.3 迁移 FileEntryViewSet 继承到 StandardCacheMixin
- [ ] 3.4 运行 FileEntryViewSet 单元测试验证迁移
...
- [ ] 4.5 运行 signals.py 集成测试验证缓存失效逻辑
...
- [ ] 6.1 从 `common/utils/cache.py` 删除 `get_cache_key()` 函数
- [ ] 6.2 从 `common/mixins/cache_mixin.py` 删除 `CacheListMixin` 类
- [ ] 6.3 从 `common/mixins/cache_mixin.py` 删除 `CacheRetrieveMixin` 类
- [x] 6.4 保留 `InvalidateCacheMixin` 类（仍被使用）
- [x] 6.4.1 更新 `InvalidateCacheMixin` 使用 `get_standard_cache_key()` 替代 `get_cache_key()`
- [ ] 6.5 删除所有测试中对 `get_cache_key()` 的引用
- [ ] 6.6 删除所有测试中对旧 Mixin 的 mock 引用
- [ ] 6.7 运行完整测试套件验证清理后无遗留问题

## 7. 文档和代码注释

- [x] 7.1 为 StandardCacheListMixin 添加 docstring 说明
- [x] 7.2 为 StandardCacheRetrieveMixin 添加 docstring 说明
- [x] 7.3 为 `_is_user_specific_queryset()` 添加详细注释
- [x] 7.4 在所有迁移的 ViewSet 中添加 TODO 注释标记已迁移
- [x] 7.5 更新 `common/mixins/cache_mixin.py` 模块级文档字符串
