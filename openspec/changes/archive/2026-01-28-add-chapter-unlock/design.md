...
List of chapter order numbers that must be completed before this chapter unlocks.
...
- Circular dependencies should be detected and rejected (validated at platform level)
...
ISO 8601 datetime string before which the chapter remains locked.
...
**Behavior**: Chapter is immediately accessible to all enrolled students (no unlock condition needed).
...
| Percentage completion | Supports `minimum_percentage` | Not applicable (all-or-nothing) |

## Implementation Notes

### Parser Validation

A markdown parser validating chapter unlock conditions should check:
...
Platforms importing chapter content should handle unlock conditions in two phases:
...
Chapter files without `unlock_conditions` field remain unlocked. This ensures existing course repositories continue to work without modification.
