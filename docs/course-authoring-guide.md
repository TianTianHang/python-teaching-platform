...
**顺序解锁**是课程章节的推荐默认模式，确保学生按顺序完成章节，每个新章节需要完成前一章节后解锁。
...
title: "Python简介与环境搭建"
order: 1
# 第一章作为课程入口点，不需要 unlock_conditions
---
```
...
**1. 无限制解锁（默认）**
```yaml
# 不设置 unlock_conditions，或明确设置为 none
unlock_conditions:
  type: "none"
```
...
以下情况可以考虑非顺序解锁模式：
- **选修章节**：部分内容可选学习，不依赖前置章节
- **并行章节**：同一难度级别的多个主题，可以任意顺序学习
- **回顾章节**：用于复习总结，应在所有内容章节之后
- **分支课程**：学生可根据兴趣选择不同学习路径

使用非顺序模式时，请在课程设计文档中说明理由。
...
- 潜在问题警告
```markdown
:::warning{title="内存管理警告"}
大列表使用时要注意内存占用，避免不必要的复制操作。
:::
```
...
:::tip{title="提示 1" state="collapsed"}
可以使用 Python 的字符串切片功能。
:::

:::tip{title="提示 2" state="collapsed"}
切片语法是 `[::-1]`。
:::
...
- ✅ `:::answer{title="完整代码实现"}`
- ❌ `:::tip{title="提示"}`（过于笼统）
- ❌ `:::warning{title="注意事项"}`（不够具体）

#### 内容组织原则

1. **保持简洁**：折叠块内内容应精炼
2. **避免嵌套**：不要在其他折叠块内使用折叠块
3. **支持其他 Markdown**：可在块内使用代码块、列表等
4. **渐进揭示**：重要信息分层次展示
...
:::tip{title="优化提示"}
Python 内置的 `max()` 函数已经做了充分优化，直接使用即可。
:::
