...
from .serializers import (
    ...
    FileEntryUpdateSerializer,
    FolderSerializer,
    UnifiedPathSerializer,
    MoveCopyPathSerializer,
)
from file_management.utils.path_utils import (
    resolve_path_to_object,
    list_path_contents,
    get_folder_by_path,
    get_file_by_path,
...
class FolderViewSet(
    ...
    def get_queryset(self):
        ...
        return queryset.select_related("owner", "parent").prefetch_related(
            "parent__owner"
        )
    ...
    def contents(self, request, pk=None):
        ...
        # Serialize the data
        files_serializer = FileEntrySerializer(
            files, many=True, context={"request": request}
        )
        folders_serializer = FolderSerializer(
            subfolders, many=True, context={"request": request}
        )
...
class FileEntryViewSet(
    ...
    def get_queryset(self):
        ...
        return queryset.select_related("owner", "folder").prefetch_related(
            "folder__owner"
        )
    ...
    def upload(self, request):
        ...
        if serializer.is_valid():
            ...
            response_serializer = FileEntrySerializer(
                file_entry, context={"request": request}
            )
    ...
    def download(self, request, pk=None):
        ...
        if (
            not file_entry.is_public
            and file_entry.owner != request.user
        ...
        try:
            ...
            if file_entry.storage_backend == "local":
                ...
                if hasattr(file_obj, "url"):
                    return Response(
                        {"redirect_url": file_obj.url}, status=status.HTTP_302_FOUND
                    )
                else:
            ...
            return Response(
                {"error": f"File not found: {str(e)}"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer = FileEntryUpdateSerializer(
...
class UnifiedFileFolderViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        try:
            ...
            if cached_data:
                ...
                files_serializer = FileEntrySerializer(
                    result["files"], many=True, context={"request": request}
                )
                folders_serializer = FolderSerializer(
                    result["folders"], many=True, context={"request": request}
                )
            ...
            response = Response(
                {"path": full_path, "files": files_data, "folders": folders_data}
            )
    ...
    def retrieve(self, request, pk=None, full_path=None):
        ...
        if path:
            ...
            path = (
                "/" + decoded_path if not decoded_path.startswith("/") else decoded_path
            )
        else:
            path = "/"
        ...
        try:
            ...
            if obj_type == "file":
                ...
                return Response({"type": "file", "data": serializer.data})
            elif obj_type == "folder":
                serializer = FolderSerializer(obj, context={"request": request})
                return Response({"type": "folder", "data": serializer.data})
        except FileNotFoundError as e:
            return Response({"error": str(e)}, status=status.HTTP_404_NOT_FOUND)
        except PermissionError as e:
            return Response({"error": str(e)}, status=status.HTTP_403_FORBIDDEN)
    ...
    def delete_path(self, request, pk=None, full_path=None):
        ...
        if not full_path and pk:
            ...
            # Ensure it starts with a slash
            full_path = (
                "/" + decoded_pk if not decoded_pk.startswith("/") else decoded_pk
            )
        ...
        try:
            ...
            if obj_type == "file":
                obj.delete()
                fresh_path = (
                    obj.folder.get_full_path() if obj.folder is not None else "/"
                )
            ...
            # If it's a folder, delete it (this will recursively delete contents)
                ...
                obj.delete()
                fresh_path = (
                    obj.parent.get_full_path() if obj.parent is not None else "/"
                )
                invalidate_dir_cache(request.user.id, fresh_path)
                return Response(
                    {
        ...
        try:
            ...
            if target_folder_path == "/" or target_folder_path == "":
                ...
                destination_folder = get_folder_by_path(
                    target_folder_path, request.user
                )
            ...
            try:
                existing_file = FileEntry.objects.get(
                    name=filename, folder=destination_folder, owner=request.user
                )
                is_update = True
            ...
            if serializer.is_valid():
                ...
                invalidate_dir_cache(
                    request.user.id,
                    target_folder_path if target_folder_path != "" else "/",
                )
                return Response(
    ...
    def create_folder(self, request):
        ...
        try:
            ...
            if not folder_name:
                return Response(
                    {"error": "Cannot create root folder"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            existing_folder = Folder.objects.filter(
                name=folder_name, parent=parent_folder, owner=request.user
            ).first()
            ...
            folder_serializer = FolderSerializer(
                new_folder, context={"request": request}
            )

            invalidate_dir_cache(
                request.user.id, parent_folder.get_full_path() if parent_folder else "/"
            )
    ...
    def move_copy(self, request):
        ...
        try:
            ...
            if source_type == "file":
                return handle_file_move_copy(
                    source_obj, dest_folder, operation, request, final_dest_name
                )
            # If source is a folder, perform folder move/copy
            elif source_type == "folder":
                return handle_folder_move_copy(
                    source_obj, dest_folder, operation, request, final_dest_name
                )
            else:
                return Response(
