...
class ChapterSerializerTestCase(TestCase):
    ...
    def test_status_returns_in_progress(self):
        ...
        ChapterProgressFactory(
            enrollment=enrollment, chapter=self.chapter, completed=False
        )
    ...
    def test_is_locked_field_with_unlock_condition(self):
        ...
        condition = ChapterUnlockConditionFactory(
            chapter=self.chapter, unlock_condition_type="date", unlock_date=future_date
        )
    ...
    def test_prerequisite_progress_field(self):
        ...
        condition = ChapterUnlockConditionFactory(
            chapter=self.chapter, unlock_condition_type="prerequisite"
        )
        ...
        context = {
            "request": type(
                "Request", (), {"user": self.user, "is_authenticated": True}
            )
        }
...
class ProblemSerializerTestCase(TestCase):
    ...
    def test_unlock_condition_description_with_prerequisite(self):
        ...
        unlock_condition = ProblemUnlockConditionFactory(
            problem=self.problem, unlock_condition_type="prerequisite"
        )
        ...
        self.assertEqual(
            len(data["unlock_condition_description"]["prerequisite_problems"]), 1
        )
...
class AlgorithmProblemSerializerTestCase(TestCase):
    ...
    def test_sample_cases_includes_only_sample_cases(self):
        ...
        CourseTestCaseFactory(problem=self.algorithm_problem, is_sample=True)
        CourseTestCaseFactory(problem=self.algorithm_problem, is_sample=False)
...
class FillBlankProblemSerializerTestCase(TestCase):
    ...
    def test_content_with_blanks_field(self):
        ...
        data = self.serializer.data
        self.assertEqual(
            data["content_with_blanks"], self.fillblank_problem.content_with_blanks
        )
...
class EnrollmentSerializerTestCase(TestCase):
    ...
    def test_progress_percentage_with_completed_chapters(self):
        ...
        ChapterProgressFactory(enrollment=enrollment, chapter=chapter1, completed=True)
        ChapterProgressFactory(enrollment=enrollment, chapter=chapter2, completed=False)
...
class ChapterProgressSerializerTestCase(TestCase):
    ...
    def test_course_title_field(self):
        ...
        data = self.serializer.data
        self.assertEqual(
            data["course_title"], self.chapter_progress.chapter.course.title
        )
    ...
    def test_completed_at_when_completed(self):
        ...
        progress = ChapterProgressFactory(completed=True, completed_at=timezone.now())
...
class ProblemProgressSerializerTestCase(TestCase):
    ...
    def test_problem_difficulty_field(self):
        ...
        data = self.serializer.data
        self.assertEqual(
            data["problem_difficulty"], self.problem_progress.problem.difficulty
        )
...
class DiscussionReplySerializerTestCase(TestCase):
    ...
    def test_validate_without_thread_field(self):
        ...
        serializer = DiscussionReplySerializer(
            data={"content": "Test content"}, context=context
        )
        self.assertFalse(serializer.is_valid())
...
class ExamSubmitSerializerTestCase(TestCase):
    ...
    def test_validate_answers_requires_problem_id(self):
        ...
        serializer = ExamSubmitSerializer(
            data={"answers": [{"problem_type": "choice", "choice_answers": ["A"]}]}
        )
        self.assertFalse(serializer.is_valid())
    ...
    def test_validate_valid_choice_answer(self):
        ...
        serializer = ExamSubmitSerializer(
            data={
                "answers": [
                    {"problem_id": 1, "problem_type": "choice", "choice_answers": ["A"]}
                ]
            }
        )
        self.assertTrue(serializer.is_valid())
...
class CrossFieldValidationTestCase(TestCase):
    ...
    def test_multiple_choice_requires_list_answer(self):
        ...
        problem = ProblemFactory(type="choice")
        choice_problem = ChoiceProblemFactory(
            problem=problem, is_multiple_choice=False, correct_answer="A"
        )
...
class ChapterSerializerPerformanceTestCase(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(1, 8, 2):  # Chapters 1, 3, 5, 7
            condition = ChapterUnlockConditionFactory(
                chapter=self.chapters[i], unlock_condition_type="prerequisite"
            )
        ...
        for chapter in self.chapters:
            ...
            if data["unlock_condition"]:
                ...
                if data["unlock_condition"]["prerequisite_titles"]:
                    self.assertTrue(
                        len(data["unlock_condition"]["prerequisite_titles"]) > 0
                    )
    ...
    def test_chapter_list_with_prefetch_verify(self):
        ...
        self.assertLessEqual(
            len(progress_queries),
            2,
            f"Too many progress queries: {len(progress_queries)}",
        )

        # Verify no N+1 pattern for enrollment queries
        enrollment_queries = [q for q in queries if "enrollment" in q["sql"]]
        self.assertLessEqual(
            len(enrollment_queries),
            2,
            f"Too many enrollment queries: {len(enrollment_queries)}",
        )
        ...
        self.assertLessEqual(
            len(unlock_queries),
            2,
            f"Too many unlock condition queries: {len(unlock_queries)}",
        )
    ...
    def test_chapter_serializer_fallback_to_service_layer(self):
        ...
        serializer = ChapterSerializer(
            chapter, context={"request": type("Request", (), {"user": self.user})()}
        )
        ...
        self.assertTrue(
            is_locked, "Chapter should be locked via Service layer fallback"
        )
...
# =============================================================================
# Global Data Cache Serializers (Phase 1.5 - Task 1.5)
# =============================================================================


class ChapterGlobalSerializerTestCase(TestCase):
    """Test cases for ChapterGlobalSerializer."""

    def setUp(self):
        """Set up test fixtures."""
        self.course = CourseFactory()
        self.chapter = ChapterFactory(course=self.course, order=1)
        self.serializer = ChapterGlobalSerializer(self.chapter)

    def test_contains_expected_fields(self):
        """Test that serializer contains only global fields (no user status fields)."""
        data = self.serializer.data
        expected_fields = {
            "id",
            "course",
    ...
    def test_excludes_user_status_fields(self):
        """Test that serializer does not contain user-specific fields."""
        data = self.serializer.data
        excluded_fields = ["status", "is_locked", "prerequisite_progress"]
        for field in excluded_fields:
            self.assertNotIn(field, data)

    def test_course_field_returns_course_id(self):
        """Test that course field returns the course ID."""
        data = self.serializer.data
        self.assertEqual(data["course"], self.course.id)

    def test_course_title_returns_course_title(self):
        """Test that course_title returns the course's title."""
        data = self.serializer.data
        self.assertEqual(data["course_title"], self.course.title)

    def test_serializes_multiple_chapters(self):
        """Test serializing multiple chapters."""
        chapters = [ChapterFactory(course=self.course, order=i) for i in range(2, 5)]
        serializer = ChapterGlobalSerializer(chapters, many=True)
        data = serializer.data

        self.assertEqual(len(data), 3)  # 3 new chapters
        self.assertTrue(all("id" in item for item in data))
        self.assertTrue(all("title" in item for item in data))


class ProblemGlobalSerializerTestCase(TestCase):
    """Test cases for ProblemGlobalSerializer."""
    ...
    def setUp(self):
        ...
        self.serializer = ProblemGlobalSerializer(self.problem)
    ...
    def test_excludes_user_status_fields(self):
        """Test that serializer does not contain user-specific fields."""
        data = self.serializer.data
        excluded_fields = ["status", "is_unlocked", "unlock_condition_description"]
        for field in excluded_fields:
            self.assertNotIn(field, data)

    def test_chapter_field_returns_chapter_id(self):
        """Test that chapter field returns the chapter ID."""
        data = self.serializer.data
        self.assertEqual(data["chapter"], self.chapter.id)

    def test_chapter_title_returns_chapter_title(self):
        """Test that chapter_title returns the chapter's title."""
        data = self.serializer.data
        self.assertEqual(data["chapter_title"], self.chapter.title)

    def test_chapter_title_returns_none_for_orphan_problem(self):
        """Test that chapter_title returns None for problems without chapters."""
        orphan_problem = ProblemFactory(chapter=None)
        serializer = ProblemGlobalSerializer(orphan_problem)
        data = serializer.data

        self.assertIsNone(data["chapter_title"])

    def test_serializes_multiple_problems(self):
        """Test serializing multiple problems."""
        problems = [
            ProblemFactory(chapter=self.chapter, type=f"type_{i}") for i in range(3)
        ]
        serializer = ProblemGlobalSerializer(problems, many=True)
        data = serializer.data

        self.assertEqual(len(data), 3)  # 3 new problems
        self.assertTrue(all("id" in item for item in data))
        self.assertTrue(all("type" in item for item in data))
