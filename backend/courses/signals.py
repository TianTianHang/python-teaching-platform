...
def invalidate_unlock_condition_cache(sender, instance, **kwargs):
    ...
    enrollments = Enrollment.objects.filter(course=chapter.course)
...
def update_exam_total_score_on_save(sender, instance, created, **kwargs):
    ...
    # 检查是否应该跳过信号（Exam 创建时）
    if (
        hasattr(instance.exam, "_skip_exam_problem_signals")
        and instance.exam._skip_exam_problem_signals
    ):
        return
...
def on_chapter_progress_change(sender, instance, **kwargs):
    """
    用户章节进度变化 → 失效用户状态缓存和章节解锁缓存

    当用户完成章节或更新章节进度时，失效该用户在该课程下的章节状态缓存
    和章节解锁状态缓存。不影响全局数据缓存，其他用户的缓存不受影响。
    """
    from django.core.cache import cache
    ...
    logger.debug(
        f"Invalidated chapter status cache and unlock cache for user {user_id} in course {course_id}"
    )
