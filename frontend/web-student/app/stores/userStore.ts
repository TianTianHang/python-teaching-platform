...
interface UserState {
    ...
    register: ({ username, password }: { username: string, password: string, stNumber:string }) => void;
...
export const useUserStore = create<UserState>()((set, get) => ({
    ...
    register: async ({ username, password ,stNumber}) => {
        const { user, access, refresh } = await http.post<{
            user: User,
            access: string,
            refresh: string
        }>("auth/register", { username, password,st_number:stNumber },{skipNotification:true})
