...
class SubmissionScenario(SequentialTaskSet):
    ...
    def step3_get_user_info(self):
        ...
        if response.status_code == 200:
            ...
            print(
                f"用户 {self.username}: 获取用户信息失败，状态码: {response.status_code}"
            )
            self.interrupt()
    ...
    def step4_set_session(self):
        ...
        if response.status_code in [200, 302]:
            print(
                f"用户 {self.username}: 设置 session 成功，状态码: {response.status_code}"
            )
        else:
            print(
                f"用户 {self.username}: 设置 session 失败，状态码: {response.status_code}"
            )
            self.interrupt()
...
class ProblemUserScenario(SequentialTaskSet):
    ...
    @task(1)
    def step6_5_get_problems_api(self):
        """步骤6.5：获取问题列表（问题页 client loader）"""
        response = self.client.get(
    ...
    def step7_get_problem_detail(self):
        """步骤7：打开一个具体问题"""
        problem_id = random.choice([1, 2, 3, 4, 5])
        response = self.client.get(f"/problems/{problem_id}", name="GET /problems/{id}")
        print(
            f"用户 {self.username}: 打开问题 {problem_id}，状态码: {response.status_code}"
        )

    @task(1)
    def step7_5_get_problem_detail_api(self):
        """步骤7.5：获取问题详情（问题详情页 client loader）"""
        problem_id = random.choice([1, 2, 3, 4, 5])
        response = self.client.get(
            f"{BACKEND_URL}/problems/{problem_id}",
            headers={"Authorization": f"Bearer {self.access_token}"},
...
class CourseUserScenario(SequentialTaskSet):
    ...
    def on_start(self):
        ...
        self.username = self.account["username"]
        self.password = self.account["password"]
        self.course_id = DEFAULT_COURSE_ID
        self.chapter_id = DEFAULT_CHAPTER_ID
    ...
    @task(1)
    def step6_5_get_courses_api(self):
        """步骤6.5：获取课程列表（课程页 client loader）"""
        response = self.client.get(
    ...
    def step7_get_course_detail(self):
        """步骤7：打开一个具体课程"""
        response = self.client.get(
            f"/courses/{self.course_id}", name="GET /courses/{id}"
        )
        print(
            f"用户 {self.username}: 打开课程 {self.course_id}，状态码: {response.status_code}"
        )

    @task(1)
    def step7_5_get_course_detail_api(self):
        """步骤7.5：获取课程详情（课程详情页 client loader）"""
        response = self.client.get(
    ...
    @task(1)
    def step7_6_get_course_enrollment(self):
        """步骤7.6：获取课程注册信息（课程详情页 client loader）"""
        response = self.client.get(
    ...
    def step9_get_chapter(self):
        ...
        response = self.client.get(
            ...
            name="GET /courses/{id}/chapters/{self.chapter_id}",
        )
        print(
            f"用户 {self.username}: 打开课程 {self.course_id} 的 chapter {self.chapter_id}，状态码: {response.status_code}"
        )

    @task(1)
    def step9_5_get_chapter_unlock_status(self):
        """步骤9.5：获取章节解锁状态（章节详情页 client loader）"""
        response = self.client.get(
    ...
    @task(1)
    def step9_6_get_chapter_detail_api(self):
        """步骤9.6：获取章节详情（章节详情页 client loader）"""
        response = self.client.get(
    ...
    @task(1)
    def step9_7_get_chapter_problems_api(self):
        """步骤9.7：获取章节问题（章节详情页 client loader）"""
        response = self.client.get(
...
class ProfileBrowsingScenario(SequentialTaskSet):
    ...
    def step6_get_user_info_again(self):
        ...
        print(
            f"用户 {self.username}: 获取用户信息（profile），状态码: {response.status_code}"
        )
...
class ComprehensiveScenario(SequentialTaskSet):
    ...
    def step13_get_chapters_list(self):
        """步骤13：进入课程章节列表页面"""
        response = self.client.get(
            f"/courses/{self.course_id}/chapters", name="GET /courses/{id}/chapters"
        )
    ...
    def step14_get_chapters_list_api_1(self):
        ...
        print(
            f"用户 {self.username}: 获取课程详情（章节列表），状态码: {response.status_code}"
        )
    ...
    def step21_get_problem_detail(self):
        """步骤21：进入问题详情"""
        response = self.client.get(
            f"/problems/{self.problem_id}", name="GET /problems/{id}"
        )
